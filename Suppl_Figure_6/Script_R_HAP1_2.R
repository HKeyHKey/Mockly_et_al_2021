library(plotrix)
#setwd("~/Documents/Projet_miR_proliferation/Cell_proliferation_assays/HAP_miR34a_wt_vs_mutants_Assay_2/")
m1 <- read.csv("SM26_m1.csv", header = F, sep = "\t", dec = ",")
m2 <- read.csv("SM26_m2.csv", header = F, sep = "\t", dec = ",")
m3 <- read.csv("SM26_m3.csv", header = F, sep = "\t", dec = ",")
m4 <- read.csv("SM26_m4.csv", header = F, sep = "\t", dec = ",")
m5 <- read.csv("SM26_m5.csv", header = F, sep = "\t", dec = ",")
m6 <- read.csv("SM26_m6.csv", header = F, sep = "\t", dec = ",") #On a atteint le plateau

mutant_2_raw <- cbind(m1[3:6, 3],m2[3:6, 3],m3[3:6, 3],m4[3:6, 3],m5[3:6, 3],m6[3:6, 3])
mutant_6_raw<- cbind(m1[3:6, 5],m2[3:6, 5],m3[3:6, 5],m4[3:6, 5],m5[3:6, 5],m6[3:6, 5])
mutant_11_raw<- cbind(m1[3:6, 7],m2[3:6, 7],m3[3:6, 7],m4[3:6, 7],m5[3:6, 7],m6[3:6, 7])
mutant_12_raw<- cbind(m1[3:6, 9],m2[3:6, 9],m3[3:6, 9],m4[3:6, 9],m5[3:6, 9],m6[3:6, 9])
wt_3_raw<- cbind(m1[3:6, 4],m2[3:6, 4],m3[3:6, 4],m4[3:6, 4],m5[3:6, 4],m6[3:6, 4])
wt_4_raw<- cbind(m1[3:6,6],m2[3:6, 6],m3[3:6, 6],m4[3:6, 6],m5[3:6, 6],m6[3:6, 6])
wt_5_raw<- cbind(m1[3:6, 8],m2[3:6, 8],m3[3:6, 8],m4[3:6, 8],m5[3:6, 8],m6[3:6, 8])
wt_14_raw<- cbind(m1[3:6, 10],m2[3:6, 10],m3[3:6, 10],m4[3:6, 10],m5[3:6, 10],m6[3:6, 10])

mutant_6_raw[1, 2] <- NA #J'ai mis trop de cellules
  
time <- c(0,9,24,34,48,57)
...
#y_range <- range(pretty(range(c(wt_3_m + wt_3_se, wt_3_m - wt_3_se,mutant_12_m + mutant_12_se, mutant_12_m -mutant_12_se), na.rm = T)))
y_range=c(0,3.5)
x_range <- c(0, 60)

pdf('Proliferation_HAP1_KO_WT.pdf',width=6,height=6)
